...
TELEGRAM_TOKEN = os.environ.get("TELEGRAM_TOKEN", "REDACTED_TOKEN")
...
def _translate_news_title(title):
    ...
    pairs = [
        ...
        ("gold hits", "طلا رکورد زد"),
        ("record high", "رکورد تاریخی"),
        ("Record High", "رکورد تاریخی"),
        ("all time high", "بالاترین تاریخی"),
        ...
        ("rate cut", "کاهش نرخ"),
        ("Rate Cut", "کاهش نرخ"),
        ("rate hike", "افزایش نرخ"),
        ("stock market", "بازار بورس"),
        ("Stock Market", "بازار بورس"),
        ("wall street", "وال استریت"),
        ("Wall Street", "وال استریت"),
        ("safe haven", "پناهگاه امن"),
        ("global market", "بازار جهانی"),
        ("Global Market", "بازار جهانی"),
        ("global markets", "بازارهای جهانی"),
        ...
        ("consumer price", "قیمت مصرفکننده"),
        ("core inflation", "تورم اصلی"),
        ("Core Inflation", "تورم اصلی"),
        ("price surge", "جهش قیمت"),
        ("price drop", "سقوط قیمت"),
        ("sell off", "فروش گسترده"),
        ("Sell-Off", "فروش گسترده"),
        ("market crash", "سقوط بازار"),
        ("market rally", "صعود بازار"),
        ("Market Rally", "صعود بازار"),
        ("crypto market", "بازار رمزارز"),
        ("breaking news", "خبر فوری"),
        ("rate cuts", "کاهش نرخ"),
        ("rate hikes", "افزایش نرخ"),
        ("hits record high", "رکورد تاریخی زد"),
        ("reaches record high", "به رکورد تاریخی رسید"),
        ("rush to safe haven", "به پناهگاه امن هجوم برد"),
        ("rush to", "به سمت"),
        ("warn of risks", "خطرات را هشدار داد"),
        ("warns of risks", "خطرات را هشدار داد"),
    ]
    pairs.sort(key=lambda x: len(x[0]), reverse=True)
    singles = [
        (r"\brallies\b", "صعود کرد"),
        (r"\bRallies\b", "صعود کرد"),
        (r"\brallied\b", "صعود کرد"),
        (r"\bsoars?\b", "صعود"),
        (r"\bSoars?\b", "صعود"),
        (r"\bslumps?\b", "سقوط کرد"),
        (r"\bSlumps?\b", "سقوط کرد"),
        (r"\bplummets?\b", "سقوط کرد"),
        (r"\bPlummets?\b", "سقوط کرد"),
        (r"\bcollapses?\b", "سقوط کرد"),
        (r"\bregains?\b", "بازیابی کرد"),
        (r"\bretreats?\b", "عقب‌نشینی"),
        (r"\bdestabilize\b", "بی‌ثبات کند"),
        (r"\bdisorderly\b", "بی‌نظام"),
        (r"\bexpectations?\b", "انتظارات"),
        (r"\bvolatility\b", "نوسان"),
        (r"\bVolatility\b", "نوسان"),
        (r"\bstrengthens?\b", "تقویت شد"),
        (r"\bweakens?\b", "ضعیف شد"),
        (r"\bWeakens?\b", "ضعیف شد"),
        (r"\banalysts?\b", "تحلیلگران"),
        (r"\binvestors?\b", "سرمایه‌گذاران"),
        (r"\bofficials?\b", "مقامات"),
        (r"\buncertainty\b", "بی‌اطمینانی"),
        (r"\bjitters?\b", "تردید"),
        (r"\bsanctions?\b", "تحریم‌ها"),
        (r"\bSanctions?\b", "تحریم‌ها"),
        ...
        (r"\bstocks?\b", "سهام"),
        (r"\bStocks?\b", "سهام"),
        (r"\bsignals?\b", "سیگنال داد"),
        (r"\bsentiment\b", "احساسات"),
        (r"\bSentiment\b", "احساسات"),
        (r"\brecession\b", "رکود"),
        (r"\bRecession\b", "رکود"),
        (r"\bbearish\b", "نزولی"),
        (r"\bbullish\b", "صعودی"),
        (r"\bslips?\b", "کاهش یافت"),
        (r"\bSlips?\b", "کاهش یافت"),
        (r"\bsinks?\b", "سقوط کرد"),
        (r"\btumbles?\b", "سقوط کرد"),
        (r"\bwarns?\b", "هشدار داد"),
        (r"\bWarns?\b", "هشدار داد"),
        (r"\btrading\b", "معاملات"),
        (r"\bjumps?\b", "افزایش یافت"),
        (r"\bJumps?\b", "افزایش یافت"),
        (r"\bdrops?\b", "کاهش یافت"),
        (r"\bDrops?\b", "کاهش یافت"),
        (r"\bfalls?\b", "کاهش یافت"),
        (r"\bFalls?\b", "کاهش یافت"),
        (r"\bsurges?\b", "افزایش شدید"),
        (r"\bSurges?\b", "افزایش شدید"),
        (r"\bBitcoin\b", "بیتکوین"),
        (r"\bbitcoin\b", "بیتکوین"),
        (r"\bEthereum\b", "اتریوم"),
        (r"\bTreasury\b", "خزانه‌داری"),
        (r"\btreasury\b", "خزانه‌داری"),
        (r"\bEmployment\b", "اشتغال"),
        (r"\bemployment\b", "اشتغال"),
        (r"\bDollars?\b", "دلار"),
        (r"\bdollars?\b", "دلار"),
        (r"\binflation\b", "تورم"),
        ...
        (r'\bnarrative\b', 'روایت'),
        (r'\bsupply\b', 'عرضه'),
        (r'\bSupply\b', 'عرضه'),
        (r'\bconcerns?\b', 'نگرانی'),
        (r'\bconcerns\b', 'نگرانی'),
        (r'\bahead\b', 'پیش‌رو'),
        (r'\bAhead\b', 'پیش‌رو'),
        (r'\bmajor\b', 'اصلی'),
        (r'\bMajor\b', 'اصلی'),
        (r'\bcurrencies?\b', 'ارزها'),
        (r'\bCurrencies?\b', 'ارزها'),
        (r'\bturmoil\b', 'آشوب'),
        (r'\bTurmoil\b', 'آشوب'),
        (r'\bspillover\b', 'سرایت'),
        (r'\bSpillover\b', 'سرایت'),
        (r'\byen\b', 'ین'),
        (r'\bYen\b', 'ین'),
        (r'\bU\.S\b', 'آمریکا'),
        ...
        (r'\band\b', 'و'),
        (r'\bAnd\b', 'و'),
        (r'\bagainst\b', 'در برابر'),
        (r'\bAgainst\b', 'در برابر'),
        (r'\bpushes?\b', 'فشار می\u200cدهد'),
        (r'\bhow\b', 'چگونه'),
        (r'\bwhy\b', 'چرا'),
        (r'\bwhat\b', 'چه'),
        (r'\bwhen\b', 'چه زمانی'),
