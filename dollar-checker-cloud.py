...
# ============================================================
def fetch_tse_index():
    """Fetch Tehran Stock Index from multiple sources with fallback."""
    import time
    # Source 1: tgju.org
        ...
        try:
            ...
            for sv in spans:
                try:
                    ...
                    pass
            if len(nums) > 15 and nums[1] > 1_000_000:
                return {
                    "index": int(nums[1]),
                    "change": int(nums[15]) if nums[15] > 0 else 0,
                    "equal_weighted": int(nums[7]) if len(nums) > 7 and nums[7] > 1_000_000 else 0,
                    "equal_weighted_change": int(nums[17]) if len(nums) > 17 and nums[17] > 0 else 0,
                }
            if attempt == 0:
                time.sleep(2)
        except Exception as e:
            print(f"  TSE tgju error (attempt {attempt+1}): {e}", file=sys.stderr)
            if attempt == 0:
                time.sleep(2)

    # Source 2: tsetmc.com CDN API
    for attempt in range(2):
        try:
            s = requests.Session()
            s.headers.update({"User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36"})
            r = s.get("https://cdn.tsetmc.com/api/MarketData/GetMarketOverview/1", timeout=20, verify=False)
            data = r.json()
            overview = data.get("marketOverview", {})
            idx = overview.get("indexLastValue", 0)
            if idx and idx > 1_000_000:
                return {
                    "index": int(idx),
                    "change": int(overview.get("indexChange", 0)),
                    "equal_weighted": int(overview.get("indexEqualWeightedLastValue", 0)),
                    "equal_weighted_change": int(overview.get("indexEqualWeightedChange", 0)),
                }
            if attempt == 0:
                time.sleep(2)
        except Exception as e:
            print(f"  TSE tsetmc error (attempt {attempt+1}): {e}", file=sys.stderr)
            if attempt == 0:
                time.sleep(2)
    ...
    try:
        s = requests.Session()
        s.headers.update({"User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36"})
        r = s.get("https://emofid.com/Markets/StockMarketWatch", timeout=15, verify=False)
        text = r.text
        # Try to find index in various patterns
        for pattern in [
            r'data-last-price="([\d,]+)"',
            r'"lastPrice"\s*:\s*([\d,]+)',
            r'indexLastValue["\s:]+([\d,]+)',
        ]:
            m = re.search(pattern, text)
            if m:
                val = int(m.group(1).replace(',', ''))
                if val > 1_000_000:
                    return {"index": val, "change": 0, "equal_weighted": 0, "equal_weighted_change": 0}
    except Exception as e:
        print(f"  TSE emofid error: {e}", file=sys.stderr)
